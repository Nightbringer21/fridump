...
def dump_to_file(session, base, size, error, directory):
    ...
    try:
        filename = os.path.join(directory, str(hex(base)) + '_dump.data')
        ...
        print("Oops, memory access violation! " + str(ex))
...
def main():
    ...
    parser.add_argument('-o', '--out', type=str, default=os.path.join(os.getcwd(), "dump"),
                        help='provide full output directory path.', metavar="dir")
    ...
    parser.add_argument('--max-size', type=int, help='maximum size of dump file in bytes',
    ...
    mem_access_viol = ""
    Memories = session.enumerate_ranges('r--' if args.read_only else 'rw-')
    i, l = 0, len(Memories)
    print("Starting Memory dump for %i memory ranges..." % l)
